	...
	public function setOutputDirectory($outputDirectory)
	{
		try {
			if (!file_exists($outputDirectory)) {
				$this->log("Output directory does not exist, creating: " . $outputDirectory, \Project::MSG_VERBOSE);
		...
		foreach ($this->schemaFilesets as $fs) {
			...
			foreach ($dataModelFiles as $dmFilename) {
				...
				if ($this->validate && $this->xsdFile) {
					$this->log("  Validating XML using schema " . $this->xsdFile, \Project::MSG_VERBOSE);
					if (!$dom->schemaValidate($this->xsdFile)) {
